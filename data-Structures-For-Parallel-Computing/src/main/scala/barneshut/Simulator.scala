...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    //Why the following codes didn't work ?
    // errorInf: 3.0 equaled 3, but -3.4028235E38 did not equal 3 minX and maxX are not correctly updated
    // The correct codes is above them in red color
    if(body.x < boundaries.minX) boundaries.minX = body.x else { if(body.x > boundaries.maxX) boundaries.maxX = body.x }
    if(body.y < boundaries.minY) boundaries.minY = body.y else { if(body.y > boundaries.maxY) boundaries.maxY = body.y }
 ...
 //par.aggregrate:
/* he aggregate method divides the input sequence into a number of chunks. For each of the chunks,
 * it uses the new Boundaries expression to create the accumulation value, and then folds the values
 * in that chunk calling updateBoundaries on each body, in the same way a foldLeft operation would.
...
//parBodies
  ...
  def computeSectorMatrix(bodies: Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(new SectorMatrix(boundaries, SECTOR_PRECISION))((s, body) => s += body, _ combine _)
  }
  ...
  def updateBodies(bodies: Seq[Body], quad: Quad): Seq[Body] = timeStats.timed("update") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.map(body => body.updated(quad)).toIndexedSeq  //!!! parSeq.toSeq => parSeq
                                                            //    IndexedSeq[Body] != bodies.type
  }
